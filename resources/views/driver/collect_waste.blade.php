       ...
       .setLngLat([  @json($markers->longitude ), @json($markers->latitude )    ]).setPopup(
            new mapboxgl.Popup({ offset: 25 }) 
              .setHTML(
                 ...
                 '<h3>{{$markers->name}}</h3><p>{{$markers->cont_no}}</p><p>{{$markers->dist}},{{$markers->thana}}</p><form action="{{route('collect')}} " method="post"> @csrf <input type="hidden" name="email" value="{{$markers->username}}"> <input type="submit" name = "collect" value="collect"  >'
              )
          )
       .addTo(map);
